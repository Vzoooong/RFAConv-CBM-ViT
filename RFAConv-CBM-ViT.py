...
class RFAConv(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size=3):
        ...
        self.act = nn.ReLU()
        self.pool = nn.MaxPool2d(kernel_size=16, stride=16, padding=0)  # 参数可以根据需要调整
    ...
    def forward(self, x):
        ...
        pooled_out = self.pool(conv_out)  # 应用池化层
        return self.act(self.bn(pooled_out))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=768, norm_layer=None):
        ...
        self.proj = RFAConv(in_channel=3, out_channel=768)
    ...
    def forward(self, x):
        ...
        x = self.proj(x)  # 应用卷积或其他投影操作
        #print(x.shape)      #torch.Size([8, 768, 14, 14])  torch.Size([16, 1024, 14, 14])
        x = x.flatten(2)  # 将从第二维开始的所有维度展平
        x = x.transpose(1, 2)  # 转置第二维和第三维
        x = self.norm(x)
        return x
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.act = act_layer()
        self.cb = MedianCBS(3072)  # 调整dim参数以匹配实际的特征维度
...
def vit_large_patch16_224_in21k(num_classes: int = 11, has_logits: bool = True):
    """
    ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
...
def vit_huge_patch14_224_in21k(num_classes: int = 11, has_logits: bool = True):
    """
